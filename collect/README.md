# ステップ0：自分のデータを集める（＝あなただけのテキストアーカイブを作る）

学習を始める前に、まず **自分が書いてきた文章を一か所に集めて、決まった形に整える** 必要があります。
落合も同じように、長年のツイート・note・書籍原稿・講演などを1つに集約した
「個人テキストアーカイブ」を土台にしました。

**あなたも、自分専用の個人テキストアーカイブを作るところから始めます。** ここがいちばん大事な作業です。
...
- 学習は **ネットから切り離したGPUの中**で完結させ、集めたデータや学習済みモデルを
  公開リポジトリに置かないこと。あなたのテキストアーカイブは、あなたの手元だけに。
